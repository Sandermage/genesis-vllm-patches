...
**Total PATCH_REGISTRY entries:** 318 — range covers `P1`–`P109` legacy +
`PN8`–`PN391` modern + `G4_01`–`G4_81` Gemma 4 family + sub-patches
...
| Metric | Count |
|:-------|:------|
| Total PATCH_REGISTRY entries | **318** |
| Tier=community (Apache 2.0, sndr) | **318** (all entries) |
...
| Implementation status=marker_only / placeholder / partial / retired | 34 (20 + 2 + 6 + 6) |
| Apply-loop coverage (apply_module set) | 296 / 318 = 93.1% |
...
| **PN122** | Sprint 2.6 v2 — CUDA graph dispatch trace wire-in (formerly `SPRINT26_CG_DISPATCH_TRACE`; renamed 2026-05-14 to conform to canonical `P[N]?\d+` patch ID convention) | opt-in | `GENESIS_ENABLE_PN122_CG_DISPATCH_TRACE` (legacy `GENESIS_ENABLE_SPRINT26_CG_DISPATCH_TRACE` accepted for one release) | — | Genesis-original (Sandermage). Wires the Sprint 2.6 v1 cudagraph dispatch instrumentation into the live decision path so dispatch divergences surface in boot logs instead of post-mortem diffs. Diagnostic-only; off by default. Family: observability. |
