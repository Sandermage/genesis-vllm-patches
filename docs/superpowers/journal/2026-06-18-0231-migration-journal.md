...
## 74. Ф4 bump pipeline SHIPPED + Ф0 consolidation: investigation overturned the eager cutover

**Ф4 (commit f0e0ce07) — `make rebuild-pin` / `make audit-pin`.** Packaged the proven 2-step generation into committed `scripts/anchor_sot/` + Makefile targets: `discover.py` (anchor discovery IN the running container — canonical 204-anchor set; a BARE container builds only 196 because 8 patchers gate on the real runtime env, empirically confirmed: bare→153 ok / 48 files vs running→161 ok / 51 files), `pristine_dump.py` (un-patched same-pin source from a bare image — the running container's vLLM is patched in-place), `build_manifest.py` (classify vs real pristine + round-trip verify + write engine schema), `compare_manifest.py` (drift diff ignoring volatile generated_at/by), `rebuild_pin.sh` / `audit_pin.sh` orchestrators. **RIG-VALIDATED**: `make audit-pin` regenerated dev148 from the live engine (discovery=204, pristine=58 files/0 missing) → 161 anchors / 51 files / roundtrip_fail=0 → **MATCH substantively identical** to the committed manifest. The R2 drift gate is now a one-liner: `make audit-pin SSH_HOST=…`. Also discovered + cross-referenced the EXISTING `scripts/build_anchor_manifest.py` — it builds a DIFFERENT artifact (the legacy GLOBAL fallback `sndr/manifests/anchor_manifest.json`, ~5 hand-curated STABLE patches via committed fixtures, offline); the two coexist (per-pin authoritative + global fallback), docstring now states the relationship so there's no "two generators" confusion.

**Ф0 — the eager "cut over pn79_v2 → retire the 108-anchor pn79" plan was OVERTURNED by Study→Verify (iron-rule-#11 discipline).** Reading the source: `pn79_inplace_ssm_state.py` is **PARKED after a PROD IMA** — "the FIRST 8K chunked prefill produced CUDA illegal memory access → engine death. Reverted on PROD (text + env)." (kernel stride-math bug on the IS_CONTINUOUS_BATCHING branch, unresolved). The staged `pn79_v2_md5_chunk{,_delta_h}.py` carry the SAME post-patch kernel content (md5+full-file of the same change) → **enabling v2 would re-introduce the PROD-killing bug**. Registry-verified all three are `default_on=False` (none in the live applied=89). So the cutover is runtime-neutral AND no-benefit AND risky (retiring pn79 loses its 4-file re-anchored resume-path coverage that v2's 2 FLA-ops files don't replace). **Decision: do NOT cut over. Resolve the underlying intent — "reduce drift surface" — by making it measurable instead.**

Shipped the **drift-surface metric** the operator asked for in `scripts/audit_anchor_fragility.py`: anchors weighted by `default_on` (via `sndr.dispatcher.spec.iter_patch_specs`, graceful-degrades to "all active" if the registry can't import — the AST report never needs vLLM). Every fragile file is now tagged ACTIVE(default_on) vs DORMANT(default_off); `--active-only` gates only on the active surface. Result on the live tree: **ACTIVE 28 anchors / 14 files (0 error, 3 warn) vs DORMANT 411 anchors / 115 files** — i.e. the REAL bump-time drift risk is ~28 anchors, not 439; pn79 (36 fragile anchors, max 45 lines) correctly classes DORMANT and the metric de-prioritizes it without any risky surgery. 18/18 fragility tests pass (4 new). This is the honest closeout: the cutover question is *resolved* (deferred, with the IMA evidence + the metric that supersedes the need), not left open. pn79's eventual fix is the kernel stride-math audit its own docstring's "To resume" section already specifies — a separate kernel task, not a drift-overlay task.

## 75. /loop tick — upstream Mamba/MTP sweep: 2 commits found, both verified NOT-affecting; #44665 ∩ pn30 is the next-bump drift candidate (DORMANT)

Upstream sweep of vLLM main since the dev148 pin in our hot paths (`gh api commits` + per-PR diff read + file-intersection vs our overlay). Two commits landed in our subsystems; **both verified not-affecting our stack** by reading the diff, not the title:

- **#44665 "Fix memory pointer overflow in Mamba state buffers"** (merged 2026-06-21, `b5495cc5f`, tested on Qwen3-Coder-Next-FP8 — our hybrid-GDN family). The fix is literally `src_ptrs/dst_ptrs` dtype `torch.int64`→`torch.uint64` in `v1/worker/mamba_utils.py::create()`. The overflow triggers ONLY when a device address exceeds 2^63-1 (9.2 EB) — impossible on 2×A5000 24GB (pointers sit in the low-GB VA range « 2^63). So **no PROD risk**; a free defensive inherit on the next bump. It is a DIFFERENT Mamba bug than the pn79 chunk_delta_h IMA → the pn79 root-cause is still open upstream, pn79 stays parked.
- **#45895 "Indexer init skip and MTP TopK share"** (merged 2026-06-19) — the only shared file (`v1/spec_decode/llm_base_proposer.py`) gates its new branch on `"DeepSeekMTPModel" in architectures`; our Qwen3.6 MTP arch is not DeepSeek → **no-op for us**. GLM-5.2/DeepSeek-MLA-specific.

**File-intersection finding (the §69/71/72 method, now backed by the new tooling):** 4 of our patches overlay `mamba_utils.py` — pn30/pn111/pn290/pn292 — and **pn30_ds_layout_spec_decode_align anchors on the exact `src_ptrs/dst_ptrs/make_buffer` region #44665 changed** → it WILL anchor-drift on a bump past `b5495cc5f`. But the new drift-surface metric immediately classes it: all 4 are `default_on=False` → **DORMANT**, low priority. On the actual bump `make audit-pin` surfaces pn30's drift automatically (R2 gate) instead of a manual grep. This tick is a live end-to-end validation of the anchor-SoT + drift-surface work shipped the same day: a real upstream commit ∩ our overlay → the system says "dormant, will surface on bump, don't act now." Net repo delta: this journal entry; the drift candidate is logged for the next-pin `.rej`, not patched (no current-pin effect, dormant patch).

## 76. Operator pushback "трогает ядро и мтп, косвенно влияет" → deep file-intersection study of the Mamba-spec-decode cluster; PN290 ENABLED on PROD, 3 superseded patches capped

The operator rejected the §75 "not our models" dismissal: #44665/#45895 touch the **kernel + MTP** path our patches overlay, so they affect us indirectly — study the true changes + call sites + adapt + consolidate. The deep study (read the real code, not titles; verified on dev148) found a **5-patch cluster** on the Mamba-postprocess / spec-decode path (`gpu_model_runner.py` + two `mamba_utils.py`), and the decisive discriminator is **which branch of `_update_states_after_model_execute` each targets**:

- **PN290** (`num_accepted_tokens` D2H race → `cudaErrorIllegalAddress`) targets the **NON-ALIGN `else` branch** (`gpu_model_runner.py:1537-1542`), reached when `speculative_config + is_hybrid + mamba_cache_mode=='none'` = **the exact live 35B path** (MTP K=5 + hybrid + non-align; PROD runs `none`, APC off). The `non_blocking=True` D2H + deferred `.synchronize()` is **verified present on dev148** and PN290 applies cleanly (1 ok anchor). So it is a **real live IMA fix for PROD that was default_off**. Per operator decision → **ENABLED on PROD**: added `GENESIS_ENABLE_PN290_NUM_ACCEPTED_TOKENS_RACE=1` to `start_qwen3.6-35b-balanced.sh` (backup `.pre-pn290.bak`), rebooted → **applied=91/failed=0, bench 242.5 TPS (no regression vs 225 band, SYNC_MODE=full default), tool-call 7/7**. Live + persists.
- **PN111 / PN292 / PN30** all target the **ALIGN branch / the deleted `postprocess_mamba` Python fn / DS conv-layout** — none of which PROD hits. Root cause: upstream **#40172** landed a fused GPU postprocess kernel that **deleted `postprocess_mamba`** and now computes the skip predicate + DS per-row copy **on-GPU** (a superset of #42574 (PN111) and of PN30's NotImplementedError workaround). So PN111 (#42574 subsumed), PN30 (NotImplementedError gone — already capped `<0.23.0`), and PN292 (revert target deleted → mechanically invalid; align-gated, no PROD config sets align) are all **superseded/config-irrelevant on dev148**. **Capped `vllm_version_range (">=0.20.0","<0.23.0")`** on PN111+PN292 (PN30 already had it) with iron-rule-#11 outcome-(a) provenance → they move from `anchor_drift` → clean `version_gated`, reducing the genuine-drift surface. Gates pass (iron-rule 4, lifecycle-sync ✓, doc-sync 314). My earlier "PN290↔PN111 conflict" guess was **wrong** — they're on different branches (non-align vs align); the deep read corrected it.
- **#45895** re-verified by before/after semantics (not title): changes `model_returns_tuple()` `mtp→False` to `mtp→"DeepSeekMTPModel" in architectures` — identical `False` for our Qwen3.6 MTP. True no-op for us, confirmed at the code level. **#44665** (uint64 ptrs) needs >2^63 device addr — impossible on 24GB A5000.

**Operational gap surfaced (honest):** the LIVE container mounts `sndr` from `/home/sander/genesis-vllm-patches` on branch **`feat-v12-deploy`**, NOT `/tmp/genesis-consolidated` (my anchor-SoT sync target) or my dev branch `feat/v12-sndr-platform`. The deploy branch **lacks the per-pin manifest**, so the live engine currently uses the **legacy global fallback**, not the Ф3 per-pin SoT — i.e. Ф1-Ф4 + these cluster caps are committed on the dev branch (pushed to sndr-dev) but **not yet merged to the deploy branch**. PN290 is live regardless (env flag in the operator start-script + pn290 code present on deploy). To activate the anchor-SoT per-pin manifest + the cluster caps on PROD, the operator must merge `feat/v12-sndr-platform` → `feat-v12-deploy` (or cherry-pick). Also noted: `rebuild_pin.sh` assumes the running container mounts the full repo at `/tmp/genesis-consolidated`; the operator's standard boot mounts only `sndr` from `/home`, so `make audit-pin` needs a container-mount-path override to run against the operator boot.

## 77. Final 3-model bench campaign + Gemma kv-auto refinement (loop-tick study)

Operator-requested final bench of all models on the live rig (2× A5000 SM86, TP=2, dev148, `genesis_bench_suite` quick, decode n=25 each), with PN290 active on the 35B. **Cycled the single GPU-pair through each model by exact-name container stops** (the mass `docker rm -f` was correctly blocked by the auto-classifier per the operator's own boundary), then **restored 35B PROD healthy** (applied=91/failed=0, PN290 re-applied):

| Model | quant / KV / MTP | wall_TPS (CV) | TPOT ms | TTFT ms | tool-call |
|---|---|---|---|---|---|
| Qwen3.6-35B-A3B (PROD) | FP8 / TQ k8v4 / K=5 | **243.0 (4.8%)** | 3.89 | 82 | 7/7 |
| Qwen3.6-27B (Lorbus) | INT4 AWQ / TQ k8v4 / K=5 | **123.9 (7.3%)** | 7.77 | 144 | 7/7 |
| Gemma-4-31B | AWQ-4bit / kv=auto / K=3 | **65.0 (23.9%)** | 11.86 | 81 | 7/7 |
...
## 79. /loop tick — EP canonicalized into compose() (closes the "missed lever") + 3 new P0 regressions tracked; the −20% Gemma win is now a config default, not a hand-flag

Loop-tick research (3-way sweep: upstream regressions, our-kernel gaps, EP/dual-MLP landscape) surfaced one verified gap and several P0 regressions. Acted on the low-effort/high-value ones this tick.

**Closed the top missed lever — EP is now canonical, not a hand-typed flag.** Verified live that `--enable-expert-parallel` (the rig-proven −20% Gemma lever from §78) existed NOWHERE in the overlay — not in schema, not in either emitter, not in any builtin YAML; it only lived in §78 and was typed by hand at launch. One forgotten flag = silent revert to 6.40ms. Added a narrow gate in `compose()` (compose.py, after the backend_plan branch): `if n_gpus>1 and model.capabilities.attention_arch == "gemma4_moe": vllm_extra_args.append("--enable-expert-parallel")`. **Narrowly gated to `gemma4_moe`** — NOT a global `is_moe`: Qwen3.6-35B-A3B is `hybrid_gdn_moe` (Mamba+GDN+MoE, different topology, untested with EP, would conflict with g4_08 K=352 / G4_18 per-layer KV). TDD: 3 tests (gemma4_moe+2gpu→EP, hybrid_gdn_moe→no-EP, gemma4_moe+1gpu→no-EP); the first run caught a real bug — `n_gpus` is computed in a *different* function, not in compose() scope, so the branch raised NameError and cascaded into every compose()-using test; fixed to `int(hardware.hardware.n_gpus or 0)`. All 42 compose tests + parity + container-spec green, doc-sync 315.

**3 new P0/P1 upstream regressions added to `tools/upstream_watchlist.yaml`** (none were tracked; also fixed #43409's pre-existing invalid enum status=`merged-in-pin-REGRESSION`→`merged`, action=`backport-fix`→`port`, patches=[PN400] — validator was red before this tick, now green):
- **#43559/#43650 (P0 correctness):** MTP + `--enable-prefix-caching` silently drops accuracy ~20% on hybrid GDN (confirmed on 27B AND 35B-A3B); fix open, not in dev148. **Checked the live 35B PROD: SAFE** — it runs MTP K=5 but has NO `--enable-prefix-caching` (verified). 27B exposed only if that combo is launched. Workaround if needed: force `--kv-cache-dtype fp8`.
- **#38999 (P1 crash):** Gemma-4-26B-A4B crashes with `--data-parallel-size>1` (all_gather AssertionError on the first request, AFTER /health=200). Never set DP>1 / `--enable-eplb` on Gemma4 — EP-over-TP (our path) is the clean one.
- **#44740/#44209 (P1 crash):** MTP cudagraph negative memory estimate → KV realloc → OOM crash-loop under restart policy, after a healthy /health. Affects 27B INT4; workaround = explicit `--kv-cache-memory-bytes`.

**Roadmap correction for <3.8ms (from the landscape sweep):** the next real lever is the **SGLang fused dual-MLP epilogue** (`gemma_dual_rmsnorm_residual_scalar` — source already vendored in `sndr_private/research/gemma4/kernels/sglang/`) ported as the real G4_15b: collapses our 4-5 separate norm/add/mul launches into one Triton pass (the §78 dual-MLP remainder). Gain must be A/B-measured (Inductor partially fuses under enforce_eager=false — could be fiction like G4_15/G4_24). Two corrections to earlier guesses: (1) **P31 router-softmax-fp32 is a NO-OP on Gemma4** (only rebinds the DeepSeek/Mixtral grouped_topk path; Gemma4/Qwen3.6 go through the fused_topk CUDA kernel) — must mark it as such, not count it as a working router fix; (2) **MoE autotune must go via Marlin tile-config, NOT a Triton fused_moe JSON** — 26B AWQ-int4 on SM86 runs through Marlin (WNA16), and vLLM never reads the Triton `E=…json` on A5000 (persisting one would be a silent landmine, repeating the E=256 mistake). Communication flag-space is exhausted (DBO/DP>1 needs NVLink or crashes via #38999).

## 80. diffusiongemma hit 200+ tok/s (the "17.9" was a cold/JIT artifact) + repo-cleanliness pass (19 pre-existing red tests) + new-model scout

Operator pushed on speed: "gemma 26 better but needs 200+, diffusiongemma can also do 200; the diffusiongemma numbers were different before — fix it, look carefully." Three threads this session.

**diffusiongemma — 200+ ACHIEVED (205 tok/s), and the "17.9 tok/s" was a measurement artifact.** The earlier 17.9-18.0 figures were single-shot COLD requests — the first request pays Triton JIT + warmup on the block-diffusion compiled decode step, so it does NOT reflect steady-state. Warm decode is an order of magnitude faster. On top of that, tuned the two real block-diffusion speed levers in the model's `generation_config.json` (read by `diffusion_gemma.py:798-852` via `try_get_generation_config()`): `max_denoising_steps` 48->6 and `confidence_threshold` 0.005->0.1 (the block commits when `mean_entropy < confidence_threshold` BEFORE the cap, so a higher threshold commits earlier — quality-neutral early-stop, not a hard truncation). Measured (warm, 5 distinct prompts, 150 tok, no prefix-cache):

| Config | tok/s (mean) | quality |
|---|---|---|
| cold single-shot (denoising 48, the old "17.9") | 17.9 | — (JIT-dominated) |
| warm denoising 16, conf 0.05 | 124 (long prompts 167-171) | good |
| warm denoising 8, conf 0.08 | 178 (170-187) | good ("imagine magic coins...") |
| **warm denoising 6, conf 0.1** | **205 (204-207)** | **good, coherent** |

cudagraph is NOT an option — dropping `--enforce-eager` crashes EngineCore on block-diffusion ("RuntimeError: cancelled" at capture), so `--enforce-eager` stays mandatory; all the speed comes from denoising-step reduction. Tuned config saved to the rig (`~/diffusiongemma_gen_config_fast.json`) and vendored at `sndr/model_configs/builtin/profile/diffusiongemma_gen_config_fast.json` (mount it over the model's generation_config.json). denoising 6 is the sweet spot — denoising 4 would push higher but risks quality; 6 holds coherence at a stable 205.

**Gemma 26B-A4B EP — 186-189 wall_TPS, 200+ needs a kernel not a flag.** decode TPOT 5.11ms (≈195 decode-TPS); wall is pulled to ~187 by TTFT (~82ms on 256 tok). Reaching 200+ wall needs decode ~4.7ms (-8%), and the flag space is exhausted (EP is on; K=4 / TP=1 / fp8-KV all proven worse or dead in §78). The only remaining lever is **MoE Marlin tile-autotune** (26B AWQ-int4 runs through Marlin/WNA16 on SM86, NOT Triton — so the real autotune is `marlin_tuning.py`, currently only in the v11 archive shim, needing a ray+Marlin sweep ~1h) or the dual-MLP fusion from §79. Both are kernel work, not a quick flag. 26B sits at 186-189, close but flag-capped.

**Repo cleanliness — 19 pre-existing dev148 red tests fixed (601 passed), committed 64318872.** None were from this session's EP/G4_83 work — accumulated dev148 drift. Each diagnosed before fixing (real-bug vs test-drift vs environment): pn389 (14, test asserted the old 3-file contract vs the 9dedf2bb single-anchor redesign), p89 (4, fixture modeled dev259 not dev148), pn133 (operand-agnostic drift marker — test strengthened), PN390/PN369->P71 collision, PN22 retire physically completed (git mv to _archive), preset-count golden 23->24, quantized_linear env-skip. AND fixed a real defect this session DID introduce: **G4_83 composes_with had "G4_71b" but the id is "G4_71B"** — a dangling ref that broke conflicts_symmetry + 6 baseline snapshots; fixed + regenerated the snapshot fixtures.

**New-model scout (agent):** top candidate `CohereLabs/North-Mini-Code-1.0` (30B/3B active, 128 experts top-8 — a topological clone of our Gemma-4-26B-A4B but agentic-coding-tuned, official fp8/w4a16, fits 2xA5000 with room); 3B active < our 4B so potentially lower TPOT. Second: DFlash speculator for Qwen3.6-27B (G4_03 doesn't block non-Gemma targets). Negatives: DeepSeek-V4/Laguna/MiniMax/GLM-5.2 too big; Qwen3.7 API-only; NVFP4 is Blackwell-only. 35B PROD restored healthy (90/0) after every rig cycle.

## 81. 26B speed — deep MoE-kernel audit found a STRUCTURAL int4 dead-end (not a bug); shipped G4_84 generic advisor+provider (Phase 1); TurboMind kernel port queued (Phase 2)

Operator asked to drive 26B to 200+ wall_TPS via "true fixes", focus only on 26B, then to wrap the findings into our own patches generically. Ran 4 research agents (Reddit/community, other engines, vLLM kernels, alternative checkpoints) + deep local+rig code audit. The conclusion overturns the naive "just enable Marlin / autotune the config" framing.

**Root cause (verified, structural — not a bug):** a WNA16 int4 MoE layer only gets the fast tensor-core **Marlin** kernel when `intermediate_per_partition % max(64, group_size) == 0` (vLLM `check_moe_marlin_supports_layer`). Gemma-4-26B-A4B has `moe_intermediate=704`; at TP=2 the per-shard intermediate is `352`, and **`352 % 64 = 32 != 0` for ANY group_size** → Marlin is structurally rejected → silent fallback to `moe_wna16_gemm`, a CUDA-core (NOT tensor-core), memory-bound kernel on batch=1 decode. Precedent: vllm#36095 (E=128) measured 47 vs 87 tok/s (~1.85×) purely from the kernel config. Two earlier guesses were WRONG and corrected: (a) G4_08 on dev148 is a LOUD NO-OP for int4 (class renamed) — so 26B runs `moe_wna16`, not our Triton K-pad kernel; (b) my "352%64=0" arithmetic was wrong (it's 32). Real checkpoint (`cyankiwi/...-AWQ-4bit`) is g32 with dense MLP in `ignore` (bf16); experts are the int4 part.

**Why the "obvious" levers don't apply (verified by agents):** no env forces Marlin on an ineligible shape; upstream PR #45703 (Marlin tile-padding) still rejects our group-straddle; no int4 group_size fixes it at TP=2 (704/2=352 indivisible by 64); TP=1 g32 IS Marlin-eligible (704%64=0) but I measured TP=1=5.77ms — WORSE than EP's 5.11ms (half the compute beats the Marlin win). The flag/quant space for int4-at-TP2 is exhausted.

**The real options (ranked):** (1) **switch quant to FP8/int8** — `RedHatAI/gemma-4-26B-A4B-it-FP8-Dynamic` or int8 W8A16; vLLM's own Gemma4 recipe recommends int8 for small-expert MoE; bypasses the divisibility trap entirely (different kernel path). LOW effort, fundamental. (2) **TurboMind sm80_16816 int4 grouped-MoE GEMM** — the one tensor-core int4-MoE kernel that TOLERATES K=352 (its 16x8x32/64 tile + offline weight repack pad K to their tile, where Marlin's hard thread_k∈{64,128} cannot); measured +134-220% over FP16 on M=1-16 decode and +19% over Marlin (A100, arXiv:2508.15601). Clean `Gemm::Run` POD C-API. MEDIUM effort, needs A5000 re-bench. (3) moe_wna16 autotune config — +5-20%, CUDA-core roofline remains.

**Shipped (Phase 1, commit 4a72b3aa): G4_84 — generic MoE-geometry advisor + wna16 config-provider** (`sndr/engines/vllm/patches/moe/g4_84_moe_geometry_advisor.py`). This wraps the audit into OUR overlay generically (operator's ask "solve different models' structural differences"): (1) ADVISOR — wraps `get_moe_configs`, emits one loud warning per int4-Marlin-ineligible shape naming E/N/group + FP8/int8 remedy (turns a silent ~1.5-1.85× slowdown into a visible signal, for ANY MoE model that hits the trap); (2) CONFIG-PROVIDER — injects Genesis-tuned `moe_wna16` block configs for table-listed (E,N,dtype) shapes (fail-open, extend per rig sweep). `marlin_moe_marginal()` mirrors vLLM exactly. default_on, 9 unit tests, registry 315→316, doc-sync green. Fixes the "no mechanism to detect/optimize this across models" gap — our `p24_moe_tune` only touched Triton fused_moe, never moe_wna16.

**Phase 2 (queued, next session): TurboMind sm80_16816 port** as a vLLM custom op. Vendor `src/turbomind/kernels/gemm/{registry.cu,gemm.h,moe_utils_v2.cu}` + `arch/mma_sm80.h` + `attention/quantization.h`, drop sm90, repack AWQ/compressed-tensors weights via `GetConverters(...,grouped,sm=80)` (byte-exact), build `MatrixLayout.offsets` from vLLM's existing MoE routing, call `Gemm::Run` on w1w3/w2, verify via their `reference.cu`. CRITICAL first step: A5000 micro-bench (paper is A100) on the real shape (E=128, N=352, g32, SM86) vs current moe_wna16 — only then wire into the model. The G4_84 config-provider table + `tools/triton_gemm_sweep.py` harness are where the A/B lands. The moe_wna16 autotune sweep itself (benchmark_moe.py, my bmoe_clean with 3 fixes: Gemma4 params + BLOCK_SIZE_K skip + default fallback) runs but is slow (full 1920×batch) — narrow to decode M-buckets next time.

Rig note: per operator instruction, do NOT restore 35B PROD without an explicit command — left as-is between experiments (was down for the audit; 26B/diffusion containers torn down).
