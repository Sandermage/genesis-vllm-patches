...
#
# Genesis G4_T1 — vendored gemma4 tool-parser fix overlay
# ─────────────────────────────────────────────────────────────────────────
# This file is a verbatim copy of the patched gemma4_tool_parser.py from
# noonghunna/club-3090 (2026-05-08 vendor of vLLM PR #42006 + PR #41991).
#
# Status (verified 2026-05-30):
#   - PR #41991 ([Bugfix][Gemma4] Fix infinite loop and array boundary
#     issues in tool parser) — MERGED upstream 2026-05-08T21:24:37Z.
#     Our pin 626fa9bb (2026-05-28) ALREADY includes this fix in the
#     base /usr/local/lib/python3.12/dist-packages/vllm/tool_parsers/
#     gemma4_tool_parser.py (790 lines).
#   - PR #42006 ([Bugfix] Fix Gemma4 MTP streaming multi-tool calls) —
#     STILL OPEN upstream. Our pin does NOT have it. This vendor adds
#     the 142-line refactor of `_extract_streaming` that PR #42006
#     contributes (final file: 919 lines vs 790 base).
#
...
# file as `/tmp/gemma4_tool_parser_FIXED.py` per A12 + G4_T1 session
# 2026-05-30.
#
# Retire trigger: when PR #42006 merges upstream and propagates to a
# nightly tag we pin. Track:
#
...
#     formed `<|tool_call>call:get_weather{city:"London"}` but base
#     parser returns empty tool_calls[] (parse failure on the gemma
#     sub-format).
#   - With this overlay: same emission is parsed correctly; tool_calls[]
#     populated; bench passes the corresponding case.
#   - Residual edge-case failures (model emits thought but never the
#     tool_call marker) are model-quality intrinsic limits, NOT parser
#     bugs — verified by the same residual on the dev371 base pin.
#
# Original docstring follows verbatim from PR #42006 head:
"""
