...
GENESIS_P108_MARKER = (
    "Genesis P108 MTP draft-loop stream synchronization (vllm#42603) v11.0.0"
)
...
P108_NEW = (
    ...
    "            # attention metadata or launching downstream kernels. Single-\n"
    "            # stream synchronize → minimal CPU block; avoids the global\n"
    "            # torch.accelerator.synchronize() while closing the\n"
    "            # cudaErrorIllegalAddress race observed on FlashInfer + MTP\n"
    "            # under concurrency (Qwen3.6-27B/35B). Bit-identical outputs.\n"
    "            # ════════════════════════════════════════════════════════════\n"
    "            try:\n"
