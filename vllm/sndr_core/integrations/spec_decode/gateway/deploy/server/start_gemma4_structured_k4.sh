...
# Launcher: gemma4-structured-k4 — β′-A K=4 validated structured profile.
#
# Canonical SNDR_* envs. config_hash MUST match the artifact at
#   vllm/sndr_core/integrations/spec_decode/artifacts/
#   gemma4-tq-mtp-structured-k4.json
# (model_id, kv_plan, K, drafter_backend). Operator changes
# requiring re-validation invalidate the artifact match.
set -euo pipefail

VLLM_IMAGE="${VLLM_IMAGE:-vllm/vllm-openai:nightly}"
CONTAINER_NAME="${CONTAINER_NAME:-sndr-gemma4-structured-k4}"
PORT="${PORT:-8102}"
TP="${TP:-2}"
...
GEMMA4_ASSISTANT="${GEMMA4_ASSISTANT:-/models/gemma-4-31B-it-assistant}"
...
mkdir -p /tmp/sndr_structured_launcher
cat > /tmp/sndr_structured_launcher/run.sh <<INNER_EOF
#!/bin/bash
set -e
pip install -e ${GENESIS_REPO} --no-deps --quiet 2>&1 | tail -2
rm -f /tmp/genesis_pn248_acceptance_trace.log
exec vllm serve ${GEMMA4_MODEL} \\
  ...
  --trust-remote-code \\
  --speculative-config '{"method":"mtp","num_speculative_tokens":4,"model":"${GEMMA4_ASSISTANT}","attention_backend":"FLASH_ATTN"}' \\
  --gpu-memory-utilization ${GPU_MEM_UTIL} \\
  --api-key ${API_KEY} \\
  --host 0.0.0.0 --port ${PORT} \\
  --disable-log-stats
INNER_EOF
chmod +x /tmp/sndr_structured_launcher/run.sh

docker run -d --name "${CONTAINER_NAME}" \
  --gpus all --ipc=host -p ${PORT}:${PORT} \
  --entrypoint /tmp/sndr_structured_launcher/run.sh \
  ...
  -e SNDR_ENABLE_G4_69_SKIP_LAYERS_NATIVE_BACKEND=1 \
  -e SNDR_G4_TQ_FORCE_SKIP_LAYERS=58,59 \
  -e SNDR_ENABLE_G4_70_PN259B_MIXED_ALLOC=1 \
  -e SNDR_ENABLE_G4_70_PN259B_FAIL_FAST=1 \
  -e SNDR_ENABLE_G4_70_PN259C_ROUTE_B=1 \
  -e SNDR_ENABLE_G4_71B_DRAFTER_SLIDING_TRITON=1 \
  -e SNDR_ENABLE_G4_75_DRAFTER_HEAD512_TRITON=1 \
  -e SNDR_ENABLE_G4_76_DISABLE_DRAFTER_KV_SHARING=0 \
  -e SNDR_ENABLE_G4_78_DRAFTER_TARGET_KV_BRIDGE=0 \
  -e SNDR_ENABLE_PN248_ACCEPTANCE_TRACE=1 \
  -e SNDR_ALLOW_SPEC_DECODE_KV_ADAPTER=1 \
  -v /tmp/sndr_structured_launcher:/tmp/sndr_structured_launcher:ro \
...
echo "${CONTAINER_NAME} on port ${PORT} (β′-A K=4 structured profile)"
echo "  expected guard verdict: FUNCTIONALLY_VALIDATED allowed=True"
echo "  artifact: vllm/sndr_core/integrations/spec_decode/artifacts/gemma4-tq-mtp-structured-k4.json"
