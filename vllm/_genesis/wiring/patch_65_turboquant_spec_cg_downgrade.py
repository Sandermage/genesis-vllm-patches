...
TQ_CG_SUPPORT_NEW = (
    "    # [Genesis P65] Downgrade from UNIFORM_BATCH to UNIFORM_SINGLE_TOKEN_DECODE.\n"
    "    # Under spec-decode (MTP/eagle/etc.), K+1 batches go through\n"
    "    # _prefill_attention which has a structurally wrong cudagraph capture\n"
    "    # bypass (cu_seqlens_k = cu_seqlens_q assumes first-chunk prefill,\n"
    "    # missing prior cached KV in TurboQuant compressed format). Captured\n"
    "    # kernel attends without context → drafter outputs collapse to high-bias\n"
    "    # tokens (e.g. `<tool_call>` cascade). Forcing SINGLE_TOKEN_DECODE makes\n"
    "    # spec-verify K+1 batches fall to eager execution (per-request\n"
    "    # continuation branch decompresses cache correctly). 1-token decode\n"
    "    # batches retain cudagraph capture (no continuation issue).\n"
