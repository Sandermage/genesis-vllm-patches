"""TDD test for P67 kernel v3 — multi-query + GQA + causal mask vs reference Layer 3."""
...
def main():
    ...
    from p67_kernel_v3 import p67_v3_multi_query_gqa_causal

    device = "cuda"

    test_cases = [
        # (B, K_PLUS_1, Hq, Hk, D, S_prior, name)
        (1, 4, 16,  4, 64,  64, "B1_T4_Hq16_Hk4_D64_S64_GQA4"),
        (1, 4, 64,  4, 128, 290, "B1_T4_Hq64_Hk4_D128_S290_prod_like"),
        (1, 4, 32,  4, 128, 256, "B1_T4_Hq32_Hk4_D128_S256_GQA8"),
        (2, 4, 16,  4, 64,  128, "B2_T4_Hq16_Hk4_D64_S128_GQA4"),
        (1, 2, 16,  4, 64,   8, "B1_T2_short_K1eq2_GQA4"),
    ]
    ...
    for B, K1, Hq, Hk, D, S, name in test_cases:
        ...
        k_cached = inputs["k_cached"].to(device)
        v_cached = inputs["v_cached"].to(device)
        k_chunk = inputs["k_chunk"].to(device)
        v_chunk = inputs["v_chunk"].to(device)
        ...
        ref = reference_multi_query_attention_layer3(
            ...
            scale, kv_group_size, inputs["seq_lens"]
        )

        try:
            out = p67_v3_multi_query_gqa_causal(
                q, k_cached, v_cached, k_chunk, v_chunk, seq_lens, scale
            )
        except Exception as e:
