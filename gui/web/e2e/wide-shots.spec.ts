...
test("containers config + overview tabs", async ({ page }) => {
  ...
  await page.route("**/api/**", (route) => {
    const p = new URL(route.request().url()).pathname;
    let body: any = {};
    if (p.includes("/engine/metrics")) body = { reachable: true, metric_families: 42, metrics_url: "http://127.0.0.1:8000/metrics", kpis: { kv_cache_usage: 0.62, requests_running: 3, requests_waiting: 2, generation_toks_per_s: 184, ttft_avg_s: 0.12, tpot_avg_s: 0.018, e2e_latency_avg_s: 1.4, spec_decode_acceptance_rate: 0.72 } };
    else if (p.endsWith("/update-plan")) body = { update_available: false, mode: "manual" };
