  ...
  return (
    <div className="copilot">
      <div className="copilot-bar">
        <label className="param-field"><span><Server size={11} /> Engine</span>
          <select value={target.hostId} onChange={(e) => pickHost(e.target.value)}>
            <option value="">Daemon-local engine</option>
            {hosts.map((h) => <option key={h.id} value={h.id}>{h.label} · {h.host}:{h.engine_port || 8000}</option>)}
          </select>
        </label>
        <button className="ghost-button" onClick={() => setShowTools((v) => !v)} title="What the copilot can call (all read-only / dry-run)">
          <Wrench size={13} /> {tools.length} tools
        </button>
        {msgs.length > 0 && <button className="ghost-button" onClick={() => { setMsgs([]); setErr(null); }}><Trash2 size={13} /> Clear</button>}
        <span className="copilot-readonly"><ShieldAlert size={12} /> read-only</span>
      </div>
        ...
        <div className="copilot-tools">
          {tools.map((t) => (
            <div key={t.name} className="copilot-tool">
              <div className="copilot-tool-head"><code>{t.name}</code><span className={`copilot-tool-cat ${t.category}`}>{t.category === "plan" ? "dry-run" : "read"}</span></div>
              <p>{t.description}</p>
            </div>
          ))}
        </div>
      ...
      <div className="copilot-scroll" ref={scrollRef}>
        {msgs.length === 0 && (
          <div className="copilot-empty">
            ...
            <span>Read-only assistant for this Genesis stack. Ask about presets, patches, capacity or health — it calls the Product API and cites real numbers. It <b>proposes</b> changes; you apply them.</span>
            <div className="copilot-suggest">{SUGGESTIONS.map((s) => <button key={s} onClick={() => void send(s)}>{s}</button>)}</div>
          </div>
        )}
        {msgs.map((m, i) => (
          <div key={i} className={`copilot-msg ${m.role}`}>
            ...
            {m.steps && m.steps.length > 0 && (
              <div className="copilot-trace">
                <span className="copilot-trace-lbl"><Wrench size={11} /> called {m.steps.length} tool{m.steps.length > 1 ? "s" : ""}</span>
            ...
            {m.proposed && m.proposed.map((a, j) => (
              <div key={j} className="copilot-action">
                ...
                <button className="primary-action" onClick={() => onNavigate(a.section, a.params)}>Review &amp; apply →</button>
              </div>
            ))}
            {m.stopped === "max_steps" && <span className="copilot-note">stopped at the tool-call limit — ask a narrower question for more depth.</span>}
          </div>
        ))}
        {busy && <div className="copilot-msg assistant"><div className="copilot-msg-body copilot-thinking"><Loader2 size={14} className="spin" /> thinking…</div></div>}
      </div>
      ...
      <div className="copilot-input">
        <textarea
          ...
          placeholder="Ask the ops copilot…  (Enter to send, Shift+Enter for newline)"
          rows={2} spellCheck={false}
        />
        <button className="primary-action" onClick={() => void send()} disabled={!input.trim() || busy}>
          {busy ? <Loader2 size={14} className="spin" /> : <SendHorizontal size={14} />} Send
        </button>
      </div>
    </div>
  );
