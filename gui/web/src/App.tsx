...
function segmentsFromCounts(counts: Record<string, number>, colorMap?: Record<string, string>): DonutSegment[] {
  ...
  return (
    <div className="doctor-coverage">
      <PercentBar
        value={mapped}
        max={total}
        label="apply modules mapped"
        caption={`${mapped} of ${total} patches`}
        tone="accent"
      />
      <InfoRows
        rows={[
...
function ToastHost() {
  ...
  useEffect(() => {
    const onToast = (event: Event) => {
      ...
      window.setTimeout(() => setItems((prev) => prev.filter((item) => item.id !== detail.id)), 4200);
    };
