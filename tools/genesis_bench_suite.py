...
def http_post_stream(url: str, headers: dict, payload: dict, timeout: float = 60.0):
    """Yield (line_bytes, t_first_content_seconds_since_t0) for SSE chunks.
    First yield: (None, t_first_chunk). Subsequent: (line, None)."""
...
def test_server_up(host: str, port: int, key: str) -> dict:
    code, body = http_get(f"http://{host}:{port}/v1/models", _bearer(key))
    if code == 200:
        try:
            d = json.loads(body)
...
def test_tool_call(host: str, port: int, key: str, model: str, max_tokens: int = 1500) -> dict:
    ...
    for label, thinking, prompt in cases:
        ...
        code, resp = http_post_json(f"http://{host}:{port}/v1/chat/completions",
    ...
    for prompt_idx, prompt in enumerate(prompts):
        ...
        for run_idx in range(runs):
            payload = dict(model=model, messages=[{"role": "user", "content": prompt}],
                           max_tokens=max_tokens)
            r = http_post_stream(f"http://{host}:{port}/v1/chat/completions",
...
def test_stability_stress(host: str, port: int, key: str, model: str,
                          iterations: int, prompts: list[str], max_tokens: int) -> dict:
    """Long-running stress — N iterations × M prompts. Tracks CV and any failures."""
    flat = []
    failures = 0
    ...
    for it in range(iterations):
        for p_idx, prompt in enumerate(prompts):
            ...
            decode_tpot_ms = (decode_part * 1000) / max(1, r["completion_tokens"] - 1)
            wall_tps = r["completion_tokens"] / r["elapsed_s"] if r["elapsed_s"] else 0
            flat.append(dict(it=it, p_idx=p_idx,
                             decode_tpot_ms=round(decode_tpot_ms, 4),
                             wall_tps=round(wall_tps, 2),
                             ttft_ms=ttft))
    ...
    wall_tps = [e["wall_tps"] for e in flat]
    ttfts = [e["ttft_ms"] for e in flat if e["ttft_ms"] is not None]
    return dict(iterations=iterations, n_prompts=len(prompts),
...
def main() -> int:
    ...
    # 2. Decode bench
    print(f"\n[2/7] Decode bench ({runs} runs × {len(prompts)} prompts × {args.max_tokens})...")
    ...
    if not args.skip_multi_turn:
        print(f"\n[3/7] Multi-turn TTFT ({args.ttft_turns} turns)...")
    ...
    if stress_iters > 0 and not args.skip_stress:
        print(f"\n[4/7] Stability stress ({stress_iters} iters × {len(prompts)} prompts)...")
