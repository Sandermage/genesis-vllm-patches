...
def apply() -> tuple[str, str]:
    ...
    # 2026-05-09: vllm dev93+ has the fix already (`includes_prompt =
    # computed_prefill < prompt_lens` without `-1`). Detect this BEFORE
    # the MultiFilePatchTransaction dry-run so we report a clean
    # "upstream_merged" status instead of "DRIFT — anchor not found".
    # (Audit fix per "no stubs" rule: misleading DRIFT messages waste
    # operator time investigating non-existent regressions.)
    target = resolve_vllm_file("v1/worker/gpu/sample/prompt_logprob.py")
    if target is not None and os.path.isfile(target):
        try:
            with open(target, encoding="utf-8") as f:
                content = f.read()
            # Old (buggy) pattern: `includes_prompt = ... < prompt_lens - 1`
            # New (upstream-merged) pattern: `includes_prompt = ... < prompt_lens`
            # Look for the new pattern WITHOUT the trailing `- 1`:
            if (
                "includes_prompt = computed_prefill < prompt_lens\n"
                in content
            ) and (
                ...
                log.info(
                    "[PN52] upstream merged (vllm#41411) — `< prompt_lens - 1` "
                    "replaced by `< prompt_lens`. Self-retire."
                )
                return (
                    "skipped",
                    "upstream_merged — vllm#41411 already in this pin "
                    "(prompt_logprob.py uses `< prompt_lens` without -1)"
                )
        except OSError:
            pass
