...
# 2026-05-06 (Phase D-light): operator can override MULTIPLIER via env var
# `GENESIS_PN59_BYPASS_T_MULTIPLIER` for empirical tuning. Examples:
#   1 → threshold=256  (aggressive — streaming fires for most chunks)
#   2 → threshold=512
#   4 → threshold=1024 (DEFAULT — conservative, only for genuine long-T)
#   8 → threshold=2048 (very conservative)
# Use only if you have measured benefit on YOUR workload — overhead of
# the streaming scratch pool can exceed vanilla path savings on small T.
_BYPASS_T_MULTIPLIER_DEFAULT = 4
# FLA chunk size — pinned to upstream constant (`FLA_CHUNK_SIZE`)
_FLA_CHUNK_SIZE = 64


def _get_bypass_t_multiplier() -> int:
    """Read GENESIS_PN59_BYPASS_T_MULTIPLIER env, return default if unset/invalid.

    Cached per-process: env value read once via lru_cache equivalent. Test
    suites that need to flip the value should call _get_bypass_t_multiplier
    fresh OR set env BEFORE the first call.
    """
    raw = os.environ.get("GENESIS_PN59_BYPASS_T_MULTIPLIER", "").strip()
    if not raw:
        return _BYPASS_T_MULTIPLIER_DEFAULT
    try:
        v = int(raw)
        if v < 1 or v > 64:
            log.warning(
                "[PN59] GENESIS_PN59_BYPASS_T_MULTIPLIER=%r out of range [1,64] "
                "— using default %d",
                raw, _BYPASS_T_MULTIPLIER_DEFAULT,
            )
            return _BYPASS_T_MULTIPLIER_DEFAULT
        return v
    except ValueError:
        log.warning(
            "[PN59] GENESIS_PN59_BYPASS_T_MULTIPLIER=%r not an integer "
            "— using default %d",
            raw, _BYPASS_T_MULTIPLIER_DEFAULT,
        )
        return _BYPASS_T_MULTIPLIER_DEFAULT
...
# ─── Memory instrumentation (Phase D-deep 2026-05-06) ──────────────────
# Lightweight memory tracking around Phase A (pre-h ops) vs Phase B
# (h-allocation, what streaming actually optimizes). Env-gated:
#   GENESIS_PN59_MEM_TRACE=1     log peak allocated + churn per phase per call
#   GENESIS_PN59_MEM_TRACE_AGG=1 aggregate stats across calls + log on exit
# Use to validate optimization hypotheses BEFORE backporting kernel changes.
_MEM_TRACE_AGG: dict = {
    "calls": 0, "phase_a_peak_mib": 0.0, "phase_b_peak_mib": 0.0,
    "phase_a_alloc_mib_total": 0.0, "phase_b_alloc_mib_total": 0.0,
    "vanilla_path_peak_mib": 0.0,
}


def _mem_trace_enabled() -> bool:
    return os.environ.get("GENESIS_PN59_MEM_TRACE", "").strip().lower() in (
        "1", "true", "yes", "y", "on",
    )


def _mem_trace_agg_enabled() -> bool:
    return os.environ.get("GENESIS_PN59_MEM_TRACE_AGG", "").strip().lower() in (
        "1", "true", "yes", "y", "on",
    )


def _mem_snapshot(label: str) -> tuple[float, float]:
    """Return (allocated_mib, peak_mib) on current CUDA device, or (0.0, 0.0)
    if torch.cuda not available. Cheap call (~microseconds)."""
    try:
...
def _mem_trace_log(phase: str, before_mib: float, after_mib: float,
                   peak_mib: float, T: int) -> None:
    """Emit per-call memory trace line if instrumentation env enabled."""
    delta = after_mib - before_mib
    log.info(
        "[PN59 mem-trace] phase=%s T=%d delta=%+.1f MiB "
        "peak_after=%.1f MiB before=%.1f MiB after=%.1f MiB",
        phase, T, delta, peak_mib, before_mib, after_mib,
    )


def _mem_trace_agg_record(phase: str, peak_mib: float, alloc_mib: float) -> None:
    """Aggregate stats per phase across calls. Logged via process exit hook."""
    key_peak = f"phase_{phase}_peak_mib"
    key_total = f"phase_{phase}_alloc_mib_total"
    if peak_mib > _MEM_TRACE_AGG.get(key_peak, 0.0):
        _MEM_TRACE_AGG[key_peak] = peak_mib
    _MEM_TRACE_AGG[key_total] = _MEM_TRACE_AGG.get(key_total, 0.0) + alloc_mib
    _MEM_TRACE_AGG["calls"] = _MEM_TRACE_AGG.get("calls", 0) + 1
...
def get_mem_trace_summary() -> dict:
    ...
    """
    out = dict(_MEM_TRACE_AGG)
    if out["calls"] > 0:
        out["phase_a_alloc_mib_avg"] = (
            out["phase_a_alloc_mib_total"] / out["calls"]
        )
        out["phase_b_alloc_mib_avg"] = (
            out["phase_b_alloc_mib_total"] / out["calls"]
        )
    return out


def reset_mem_trace_summary() -> None:
    """Reset aggregator. Useful between distinct bench runs."""
    for k in list(_MEM_TRACE_AGG.keys()):
        if k == "calls":
            _MEM_TRACE_AGG[k] = 0
        else:
            _MEM_TRACE_AGG[k] = 0.0
...
def streaming_chunk_gated_delta_rule_fwd(
    ...
    *,
    ...
    threshold_T = window_nt * _FLA_CHUNK_SIZE * _get_bypass_t_multiplier()
...
def _streaming_path(
    ...
    *,
    ...
    # vs Phase B allocator pressure to inform future optimization decisions.
    # Cheap (~microseconds per call). Env-gated.
    _trace = _mem_trace_enabled()
    _agg = _mem_trace_agg_enabled()
    if _trace or _agg:
        if torch.cuda.is_available():
            torch.cuda.reset_peak_memory_stats()
        _phase_a_before, _ = _mem_snapshot("phase_a_start")
    ...
    if _trace or _agg:
        _phase_a_after, _phase_a_peak = _mem_snapshot("phase_a_end")
        if _trace:
            _mem_trace_log(
                "A", _phase_a_before, _phase_a_after, _phase_a_peak, T,
            )
        if _agg:
            _mem_trace_agg_record(
                "a", _phase_a_peak, _phase_a_after - _phase_a_before,
            )
        if torch.cuda.is_available():
            torch.cuda.reset_peak_memory_stats()
        _phase_b_before, _ = _mem_snapshot("phase_b_start")
    ...
    if _trace or _agg:
        _phase_b_after, _phase_b_peak = _mem_snapshot("phase_b_end")
        if _trace:
            _mem_trace_log(
                "B", _phase_b_before, _phase_b_after, _phase_b_peak, T,
            )
        if _agg:
            _mem_trace_agg_record(
                "b", _phase_b_peak, _phase_b_after - _phase_b_before,
            )
