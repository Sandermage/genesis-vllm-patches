...
GENESIS_P67_MARKER = "Genesis P67 TQ multi-query kernel for spec-decode K+1 v7.63.x_nopow2_gqa_v11.3.0_021x_perf_cache"
...
# dev354+ form (2026-06-04 anchor refresh): upstream REVERTED the v11.3.0
# changes — removed `mm_prefix_range_tensor` line + simplified the fast-path
# `if (...)` multi-line back to single-line `if _HAS_FLASH_ATTN and ...`.
# THIS WAS THE A11 -18% REGRESSION ROOT CAUSE on 27B Lorbus + TQ k8v4 + MTP K=3:
# P67 hook silently DRIFT-skipped → ALL multi-query spec-decode optimization
# lost → +18% TPS regression vs dev371. Anchor refresh below restores the
# hook insertion point under the new upstream shape.
P67_OLD = (
    "    ) -> torch.Tensor:\n"
    "        N, Hq, D = query.shape\n"
    "\n"
...
P67_NEW = (
    ...
    "        try:\n"
    "            # [2026-06-05 PERF v2]: cache imports on `self` so the per-call\n"
    "            # cost drops from ~55µs (from-imports + getLogger) to ~1µs\n"
    "            # (getattr + tuple unpack). On 27B's 64 layers × 200 requests,\n"
    "            # saves ~3-4ms TTFT after first request. First call per layer\n"
    "            # pays the full ~150µs setup; cached thereafter. Cache key is\n"
    ...
    "                self._genesis_p67_refs = (\n"
    "                    _genesis_p67_is_active, _genesis_p67_call, _genesis_p67_log,\n"
    "                )\n"
    "            else:\n"
    "                _genesis_p67_is_active, _genesis_p67_call, _genesis_p67_log = _genesis_p67_refs\n"
