...
# `GENESIS_LEGACY_P44` is synthetic — flag exists for registry/audit
...
# dev354+ form: upstream switched torch.zeros → torch.empty for the
# transient mixed-batch alloc (same shape, no init needed since the
# kernel writes every cell). Anchor matches new form; replacement pool
# accessor was always init-agnostic, so semantics preserved.
_OLD_MIXED_ATTN_OUT = (
