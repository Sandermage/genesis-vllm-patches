...
def _clear_triton_cache() -> tuple[bool, str]:
    ...
    2026-05-11 audit F-013 closure: uses project_paths.triton_cache_dir()
    as canonical source (honors SNDR_TRITON_CACHE_DIR / GENESIS_TRITON_
    CACHE_DIR / TRITON_CACHE_DIR env in that order). Container-side
    legacy paths kept as last-resort fallback for environments without
    env vars set.
    """
    try:
    ...
    cache_dirs = [
        ...
        "/root/.triton/cache",  # docker container conventional
    ]
    # De-dupe + drop empty
    seen: set[str] = set()
    cache_dirs = [d for d in cache_dirs if d and d not in seen and not seen.add(d)]
    for d in cache_dirs:
