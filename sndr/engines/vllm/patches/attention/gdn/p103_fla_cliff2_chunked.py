...
`apply()`. That works when the patch apply step runs in the same process as
...
def _make_chunked_wrapper(
    ...
    DA-003 (audit 2026-05-08): `import torch` previously sat at this
    builder's top — running on a torch-less host (Mac dev / CI no-torch
    smoke / install diagnostic) crashed even when nobody was about to
    invoke the wrapper. The import moved into the wrapped callable
    body so the builder itself stays cold-import-friendly.
    """
...
# dev354+ form: upstream added `core_attn_out` positional arg at the
# end of the ChunkGatedDeltaRuleFunction.apply() call site. Without
# this update the self-install hook silently soft-skipped and P103
# relied only on the in-process setattr wrap (lost on import-restart).
