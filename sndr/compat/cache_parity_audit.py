...
def _read_patch_source(patch_id: str) -> str | None:
    ...
    # New (v10) naming under integrations/: `p<NN>_*.py` / `pn<NN>_*.py`.
    candidates = list(root.rglob(f"{pid_lower}_*.py"))
    # Legacy fallback for old `wiring/patch_*.py` layout.
    if not candidates:
        bare = pid_lower.lstrip("p").lstrip("n")
        candidates = list(root.rglob(f"patch_{bare}*"))
        candidates += list(root.rglob(f"patch_*{bare}*"))
    for c in candidates:
        if c.is_file():
            try:
