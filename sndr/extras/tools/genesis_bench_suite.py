...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    p.add_argument("--prompts", choices=["short", "standard", "long", "structured", "code"], default=None)
...
PROMPT_SETS = {
    ...
    "structured": [
        "Return ONLY valid JSON describing a user profile with fields: id (uuid), name, "
        "email, age, address (street, city, country, zip), roles (array of strings), "
        "preferences (theme, language, notifications boolean). Populate realistically.",
        "Output ONLY a JSON OpenAPI 3.0 spec for a CRUD /tasks endpoint: GET list, "
        "POST create, GET/PUT/DELETE by id. Include schemas with required fields.",
        "Return ONLY a JSON array of 10 product objects: id, name, price (cents int), "
        "currency, category, stock, tags (string array), variants (size, color array).",
        "Output ONLY JSON for a Kubernetes Deployment manifest: 3 replicas of "
        "nginx:1.27, resource limits, liveness+readiness probes, ConfigMap volume mount.",
        "Return ONLY JSON: monthly sales report with 12 month entries, each with "
        "revenue (float), units (int), top_products array of 3 entries (sku, name, units).",
    ],
    "code": [
        "Implement quicksort in Python with detailed docstrings, type hints, the "
        "Lomuto partition scheme, and a randomized pivot. Include 5 test cases with "
        "assertions covering empty, single, sorted, reverse, duplicates.",
        "Write a complete binary search tree class in Python: insert, delete, find, "
        "in-order traversal, pre-order, post-order, depth, balance check. Include "
        "type hints and 8 unit tests.",
        "Implement Dijkstra's shortest-path algorithm with a binary heap in Python. "
        "Include graph builder, full type hints, edge cases (disconnected, negative "
        "rejection), and a 6-node test graph with expected distances.",
        "Write an LRU cache class in Python using OrderedDict: get/put O(1), capacity, "
        "stats (hits/misses/evictions). Add 10 test cases including capacity boundary.",
        "Implement Conway's Game of Life: Board class, step(), render(), seed patterns "
        "(glider, blinker), and 4 tests verifying known stable/oscillating patterns.",
    ],
}
