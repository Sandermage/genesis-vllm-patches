...
# ─── Phase D-light 2026-05-06 — bypass threshold env var ──────────────


class TestBypassMultiplierEnv:
    ...
    def test_out_of_range_falls_back(self, monkeypatch):
        ...
        assert _get_bypass_t_multiplier() == _BYPASS_T_MULTIPLIER_DEFAULT
        monkeypatch.setenv("GENESIS_PN59_BYPASS_T_MULTIPLIER", "0")
        assert _get_bypass_t_multiplier() == _BYPASS_T_MULTIPLIER_DEFAULT


# ─── Phase D-deep 2026-05-06 — memory instrumentation ─────────────────


class TestMemTraceInstrumentation:
    """Lightweight memory tracking, env-gated, gives empirical data for
    ...
    def test_mem_trace_default_disabled(self, monkeypatch):
        ...
        monkeypatch.delenv("GENESIS_PN59_MEM_TRACE_AGG", raising=False)
        assert _mem_trace_enabled() is False
    ...
    def test_mem_trace_summary_reset_round_trip(self):
        ...
        reset_mem_trace_summary()
        assert get_mem_trace_summary()["calls"] == 0
        # Synthesise some calls
        _mem_trace_agg_record("a", 100.0, 50.0)
        _mem_trace_agg_record("b", 200.0, 80.0)
        s = get_mem_trace_summary()
        assert s["calls"] == 2
        assert s["phase_a_peak_mib"] == 100.0
        assert s["phase_b_peak_mib"] == 200.0
        assert s["phase_a_alloc_mib_total"] == 50.0
        assert s["phase_b_alloc_mib_total"] == 80.0
        reset_mem_trace_summary()
        assert get_mem_trace_summary()["calls"] == 0
