...
"""Tests for `vllm.sndr_core.proof.bench_attach` — §6.8 bench-delta ingest.
...
class TestExtractHeadlineMetrics:
    def test_headline_subblock(self):
        from vllm.sndr_core.proof.bench_attach import extract_headline_metrics
...
class TestComputeDelta:
    def test_no_baseline_means_no_pct(self):
        from vllm.sndr_core.proof.bench_attach import compute_delta
...
class TestAttachBench:
    def test_creates_new_artefact_when_none_exists(self, tmp_path):
        ...
        from vllm.sndr_core.proof import load_proof_artefact
        from vllm.sndr_core.proof.bench_attach import attach_bench
...
class TestCLI:
    def test_cli_success_human(self, tmp_path, capsys):
        from vllm.sndr_core.cli.patches import _run_bench_attach
        bench = _write_json(tmp_path / "run.json", _bench_run())
        out = tmp_path / "patch_proof"
