#!/bin/bash
# Phase 1 — Arm B: v775 baseline + --enable-prefix-caching (REGRESSION CASE)
#
# CORRECTED DESIGN (2026-04-29):
# v775 baseline ALREADY has prefix-cache OFF by default (no --enable-prefix-caching
# arg in the launch). So we measure the prefix-cache ON impact directly.
#
# Hypothesis: enabling prefix-cache HURTS TPS due to vllm#38182 adverse interaction
# with MTP spec-decode (Eagle pop costs ~1024 recomputed tokens per cache hit).
# Expected TPS drop ~10-15% from v775 162 TPS baseline.
#
# Pair with Arm C to isolate WHO causes the regression (cache ON alone vs P83/P85 fix).
#
# Single change vs v775 baseline: ADDED `--enable-prefix-caching` to serve args.
# All Genesis env vars identical to v775.
#
# Run order:
#   1. ssh sander@192.168.1.10 'bash -s' < scripts/launch/start_v786_arm_b_no_prefix_cache.sh
#   2. wait ~3 min, verify /v1/models reachable
#   3. python3 tools/bench_decode_tpot_clean_ab.py --host 192.168.1.10 \
#        --arm-name arm_b_prefix_cache_on --runs 25 --prompts standard \
#        --out runs/phase1_arm_b.json
#   4. tool-call quality probe (4 cases) + 30-iteration stress
#   5. multi-turn TTFT probe (5 sequential same-prefix requests)
#   6. compare with arm_a baseline via --compare runs/phase1_arm_a.json runs/phase1_arm_b.json
set -euo pipefail
