#!/bin/bash
# Phase 1 — Arm C: v775 baseline + --enable-prefix-caching + GENESIS_ENABLE_P83=1 + GENESIS_ENABLE_P85=1
#
# CAKE-AND-EAT: enables prefix-cache (multi-turn TTFT win) AND P83/P85 mitigations
# for vllm#38182 adverse interaction (so MTP doesn't pay the cache hit penalty).
#
# Win condition: TPS ≥ Arm A (162) AND multi-turn TTFT < Arm A (no cache benefit).
# If TPS < Arm A: P83/P85 didn't fully neutralize the cache pop overhead.
# If quality regresses: rollback (P83/P85 hidden_state corruption — known risk).
#
# Two changes vs v775 baseline:
#   1. ADDED `--enable-prefix-caching` to serve args
#   2. ADDED `GENESIS_ENABLE_P83=1 GENESIS_ENABLE_P85=1` to env
#
# Run order:
#   1. ssh sander@192.168.1.10 'bash -s' < scripts/launch/start_v786_arm_c_p83_p85.sh
#   2. wait ~3 min, verify /v1/models reachable
#   3. python3 tools/bench_decode_tpot_clean_ab.py --host 192.168.1.10 \
#        --arm-name arm_c_p83_p85_cache_on --runs 25 --prompts standard \
#        --out runs/phase1_arm_c.json
#   4. tool-call quality probe (4 cases) + 30-iteration stress
#   5. multi-turn TTFT probe (5 sequential same-prefix requests)
#   6. compare via --compare runs/phase1_arm_a.json runs/phase1_arm_c.json
set -euo pipefail
