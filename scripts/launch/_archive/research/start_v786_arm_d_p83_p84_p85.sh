#!/bin/bash
# Phase 1 — Arm D (CORRECTED CAKE-AND-EAT): v775 + cache + P83 + P84 + P85
#
# Discovery from Arm C: P85 boot log says "Requires GENESIS_ENABLE_P84=1 +
# GENESIS_P84_HASH_BLOCK_SIZE=<N> for fine hashes to be computed in the first
# place." P83 dispatcher description: "downstream symptom — P84 is real fix".
# So Arm C (P83+P85 only) was missing the actual root-cause patch.
#
# Arm C result: wall_TPS 130.87 (vs A=183.27, B=128.07) — P85 was a no-op without P84.
#
# Three changes vs v775 baseline:
#   1. ADDED `--enable-prefix-caching` to serve args
#   2. ADDED `GENESIS_ENABLE_P83=1 GENESIS_ENABLE_P84=1 GENESIS_ENABLE_P85=1`
#   3. ADDED `GENESIS_P84_HASH_BLOCK_SIZE=16` (recommended per apply_all.py:1456)
#
# Win condition: wall_TPS ≥ Arm A (183.27) AND multi-turn TTFT < Arm A.
# If still < A: P83/P84/P85 stack insufficient → restore Arm A baseline.
set -euo pipefail
