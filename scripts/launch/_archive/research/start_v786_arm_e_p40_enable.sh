#!/bin/bash
# Phase 2 — Arm E: v775 baseline (winner Phase 1) + GENESIS_ENABLE_P40=1
#
# Single change vs v775: ADDED GENESIS_ENABLE_P40=1 (TQ k8v4 GQA grouping kernel).
# P40 = local backport of vllm#40792, predicted +15-25% TPS on Qwen3-32B GQA=8/k8v4
# (= our exact preset). Pure additive Triton kernel, fall-through to upstream
# scalar kernel if eligibility fails.
#
# Win condition: wall_TPS ≥ Arm A (183.27) + ≥ +5% AND tool-call ≥ 3/4 (no NEW failures).
# If TPS gain < 5% (within CV 9%): don't promote.
# If quality regresses (case 4 still fails OR new failure): rollback (P67 mirage memory note).
set -euo pipefail
